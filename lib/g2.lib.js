...
g2 = Object.assign(g2, {
    ...
    /**
    * returns object with { x: , y, r, w, dw }*/
    ...
    cir3pts(x1, y1, x2, y2, x3, y3, ls) {
        const dx1 = x2 - x1, dy1 = y2 - y1;
        const dx2 = x3 - x2, dy2 = y3 - y2;
        const den = dx1 * dy2 - dy1 * dx2;
        const isFinite = Math.abs(den) > Number.EPSILON;
        const lam = isFinite
            ? ((dx1 + dx2) * dx2 + (dy1 + dy2) * dy2) / den
            : 0;
        const x0 = x1 + 0.5 * dx1 - lam * 0.5 * dy1;
        const y0 = y1 + 0.5 * dy1 + lam * 0.5 * dx1;
        const r = isFinite ? Math.hypot(x1 - x0, y1 - y0) : Number.POSITIVE_INFINITY;
        //  console.log({ dx1, dy1, dx2, dy2, den, lam, x0, y0, r });
        ls = ls ? ls : "black";
        return { x: x0, y: y0, r, ls: ls };
    }
})
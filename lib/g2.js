...
g2.prototype = {
    ...
    ply({ pts, format, closed, x, y, w }) {
        arguments[0].pts = g2.poly.proxy(arguments[0].pts, format, x || y || w ? arguments[0] : undefined);
 ...
 */
 g2.poly.proxy = g2.poly.proxy || function(poly, format, trf) {
    if (Array.isArray(poly)) {
        ...
        return  trf ? g2.poly.proxy.trf(ply,trf) : ply;
    }
}
/**
 ...
 */
 g2.poly.proxy.trf = g2.poly.proxy.trf || function(poly,trf) {
    if (trf) {  // test for x,y,w included ...
        const points = new Array(poly.length);
        const sw = Math.sin(trf.w), cw = Math.cos(trf.w);
console.log(trf)
        points[Symbol.iterator] = function() {
            let index = 0;
            return {
                next() {
                    if (index < points.length) {
                        const p = poly[index++];
                        return { value: {x: p.x*cw - p.y*sw + trf.x, y: p.x*sw + p.y*cw + trf.y}, done: false };
                    }
                    return { done: true };
                }
            }
        ...
        return new Proxy(poly, {
            get: (pts, key) => {
                if (key === Symbol.iterator)
                    ...
                    const p = pts[+key];
                    return {x: p.x*cw - p.y*sw + trf.x, y: p.x*sw + p.y*cw + trf.y};
                }
                else
                    return pts[key];
            }
...
g2.ifc.circular = {
    ...
    isPntOn({ x, y, eps }) {
        const ddist = (this.x - x)**2 + (this.y - y)**2 - this.r**2, reps = eps*this.r;
        return -reps < ddist && ddist < reps;
    }
}

// polygonial-like commands interface
g2.ifc.polygonial = {
    get isSolid() { return this.closed && this.fs && this.fs !== 'transparent' },
    get len() { 
        const n = this.pts.length;
        let   L = 0, p1, p2 = poly[n-1];
    ...
    get lsh() { return this.state & g2.OVER; },
    get sh() { return this.state & g2.OVER ? [0, 0, 5, "black"] : false },
    pointAt(loc) {  // numeric loc [0..1]
        const n = this.pts.length;
        let   L = this.len, p1 = poly[0], p2 = poly[n-1], lseg, lsum = 0, museg;
    ...
    hit({ x, y, eps }) {
        return this.isSolid ? g2.poly.isPntIn(this.pts, { x, y }) : this.isPntOn({ x, y, eps });
    }
}
 ...
 */
g2.prototype.box.prototype = g2.mixin(g2.ifc.point, g2.ifc.label, g2.ifc.hatch, {
    w:0,
    asPoly() {
...
// helper function
g2.cir3Pnts = function({x:x1,y:y1},{x:x2,y:y2},{x:x3,y:y3},cir) {
    cir = cir || {x:0,y:0,r:1};
...
g2.canvasHdl.prototype = {
    ...
    box({ x=0, y=0, w=0, b, h }) {
        ...
        if (!w) {
            this.ctx.fillRect(x-b/2, y-h/2, b, h);
            this.ctx.strokeRect(x-b/2, y-h/2, b, h);
        }
        else {
